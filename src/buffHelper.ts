...
export class buffHelper {
	/**
	 * BCD码字符串转换为byte数组，如220411转换为0x22,0x04,0x11
	 * @param str 长度必须是偶数
	 * @returns
	 */
	...
	/**
	 * 将byte转换为二进制字符串，如 0x02 转换为 0000 0010
	 * 注意最右边是bit0
	 * @returns
	 * @param byte
	 */
	public static byte2BinStr(byte: number): string {
		// 这里暂时只考虑一个byte大小的，因为大多数情况下都是一个字节一个字节处理的
		if (byte > 256) throw new Error('值不能大于256')
	...
	public static str2AsciiBuff(str: string): Buffer {
		...
		for (let i = 0; i < str.length; i++) retBuffer[i] = str.charCodeAt(i)
